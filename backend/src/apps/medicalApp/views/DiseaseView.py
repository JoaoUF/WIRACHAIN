...
from ..serializers import DiseaseSerializer, BulkDeleteSerializer
...
class DiseaseView(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        base_qs = Disease.objects.filter(Q(enterprise_user=enterprise_id) & Q(status=Disease.ACTIVE_STATUS)).only(
            "id",
            "name",
            "description",
        )
    ...
    def update_bulk(self, request):
        ...
        self.get_queryset().filter(id__in=updatable_ids).update(status=Disease.INACTIVE_STATUS)
